...
export default {
  ...
  methods: {
    ...
    rowCount (columns) {
      let count = 1
      count += Math.max.apply(null, columns.map(col => {
    ...
    combineCell() {
      ...
      let recursionQuery = () => {
        ...
        for (let i = 0; i < templist.length - 1; i++) {
          try {
            if (templist[0][parent] === templist[i][parent] && templist[i][parent] === templist[i + 1][parent]) {
              len++
            }
    ...
    */
    getObjectValue (object, field, index) {
      ...
      if (path.trim() !== '') {
        ...
        keys.forEach(function(key) {
          if (obj !== null && typeof obj[key] !== 'undefined' && obj[key] !== null && obj[key] !== '') {
            obj = obj[key]
          } else {
            obj = defaultValue
          }
    ...
    checkCheckboxesState () {
      ...
      let el = document.querySelectorAll('input[type=checkbox].table-th__checkbox')[0]
    ...
    getColData (field) {
      ...
      let colDatas = []
      arr.forEach(item => {
        colDatas.push(this.tableData.map(val => {
          return val[item]
        }))
      })
      return colDatas
